...
namespace SDN.UI.Entities.Sales
{
    ...
    public class CreditNoteListEntity: ViewModelBase
    {
        ...
        private DateTime? _CreditDateDateTime;
        ...
        public DateTime? CreditDateDateTime
        {
            get
            {
                return _CreditDateDateTime;
            }
            set
            {
                _CreditDateDateTime = value;
                OnPropertyChanged("CreditDateDateTime");
            }
        }
        ...
        public string CreditAmount
        {
            get
            {
                ...
                else
                {
                    if (_CreditAmount != "")
                    {
                        var balance = _CreditAmount.Replace(",", "");
                        if (Convert.ToInt32(SharedValues.decimalPlaces) == 2)
                            return Convert.ToDecimal(balance).ToString("N", new CultureInfo(SharedValues.CurrencyName));
                        if (Convert.ToInt32(SharedValues.decimalPlaces) == 3)
                            return Convert.ToDecimal(balance).ToString("N3", new CultureInfo(SharedValues.CurrencyName));
