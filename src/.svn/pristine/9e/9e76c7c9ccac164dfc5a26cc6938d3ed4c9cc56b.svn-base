...
namespace SASClient.Purchasing.ViewModel
{
    ...
    public class InvoiceDebitPaymentsViewModel:InvoiceDebitPaymentsEntity
    {
        ...
        public void PrintCommand(object param)
        {
            ...
            LstSuppliers.RemoveAll(s => s.IsSelected == false);
            IEnumerable<int> selectedsuppliers = (IEnumerable<int>)LstSuppliers.Select(v => v.SupplierID);
            SharedValues.SupplierID = selectedsuppliers;
        ...
        {
            //bool a = false;
            List<InvoiceDebitPaymentsEntity> demo = new List<InvoiceDebitPaymentsEntity>();
            if (LstSuppliers != null)
                demo = LstSuppliers.Where(x => x.IsSelected == true).ToList();
            else
                return false;
