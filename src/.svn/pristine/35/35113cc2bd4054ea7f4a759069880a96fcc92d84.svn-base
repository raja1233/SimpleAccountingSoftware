namespace SDN.Settings.DAL
{
    ...
    public class OptionsDAL : IOptionsDAL
    {
                ...
                try
                {
                    ...
                    else
                    {
                        Option optionDetails = new Option()
                        {
                            ...
                            Date_Format = optionEntity.DateFormat.Replace("mm", "MM"),
        ...
        {
            int cashType = Convert.ToInt32(Account_Type.Cash);
            int bankType = Convert.ToInt32(Account_Type.Bank);
            using (SASEntitiesEDM entites = new SASEntitiesEDM())
            {
                List<AccountsEntity> accountsource = entites.Accounts.Where(x => x.IsDeleted != true
                && (x.Acc_Type== cashType || x.Acc_Type==bankType)).Select(x => new AccountsEntity
                {
        ...
        public int GetYearStart()
        {
            ...
            {
                int mYear=0;
                var obj = entites.Options.FirstOrDefault();
                if(obj != null)
                {
                    mYear = obj.CreatedDate.Value.Year;
                }
                return mYear;
            }
        }
