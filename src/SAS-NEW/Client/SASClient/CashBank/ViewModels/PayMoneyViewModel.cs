...
namespace SASClient.CashBank.ViewModels
{
    ...
    public class PayMoneyViewModel : PayMoneyEntity
    {
        ...
        private DateTime? _PayDateCalender;
        ...
        public DateTime? PayDateCalender
        {
            get
            {
                return _PayDateCalender;
            }
            set
            {
                _PayDateCalender = value;
                OnPropertyChanged("PayDateCalender");
            }
        }
        ...
        /// </summary>
        public PayMoneyViewModel(IRegionManager regionManager, IEventAggregator eventAggregator)
            : base()
        {
            ...
            NavigateToPaymentToSupplier = new RelayCommand(NavigateToPS);
            if (!String.IsNullOrEmpty(SharedValues.NewClick))
            {
                if (SharedValues.NewClick != "New")
                {
                    ...
                    GetPayMoneyDetails(SharedValues.NewClick);
                }
        ...
        public RelayCommand NavigateToPaymentToSupplier { get; set; }
        ...
        void OnSavePS(object param)
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                ...
                PayMoneyEntity rsEntity = GetDataIntoModel();
                ...
                if (rsEntity != null)
                {
                    if (IsNew == true)
                    {
                        int i = rmRepository.SavePayMoney(rsEntity);
                        IsCashChequeEnabled = false;
                        IsTextBoxReadOnly = true;
                        IsNew = false;
                    }
                    ...
                    {
                        int i = rmRepository.UpdatePayMoney(rsEntity);
                        IsNew = false;
                        IsCashChequeEnabled = false;
                        IsTextBoxReadOnly = true;
                    }
                ...
                if (SelectedCashBankAcntID != 0 || SelectedCashBankAcntID != null)
                {
                    ...
                    if (actType != null)
                    {
                        if (actType.AccountType == Convert.ToByte(Account_Type.Cash) && actType.IsLinked == true)
                        {
                            ...
                            IsChequeFalse = true;
                            IsTextBoxReadOnly = true;
                            CashChequeNo = "PM-" + rmRepository.GetLastCashNo();
                            IsCheque = false;
                        }
                        else
                        {
        ...
        private void GetModelData(PayMoneyEntity rmf)
        {
            if (IsNew == false)
            {
        ...
        {
            string date = this.DateFormat as string;
            CashChequeDateStr = Convert.ToDateTime(this.PayDateCalender).ToString(date);
