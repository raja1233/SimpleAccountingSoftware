namespace SDN.SimpleAccounting
{
    ...
    public class SimpleAccountingViewModel : StackEntity
    {
        ...
        public void loadRefreshItem()
        {
            try {
                List<string> listview = (List<string>)Application.Current.Resources["views"];
                var secondlast = listview.AsEnumerable().Reverse().First();
                CloseView RefreshView = new CloseView(regionManager, eventAggregator);
                IEnumerable<object> prismviewlist = this.regionManager.Regions[RegionNames.MainRegion].Views;
                var itemview = prismviewlist.Last().ToString();
                if(itemview != "SDN.Navigation.View.MainView")
                {
                    RefreshView.CloseViewRequest(secondlast, false);
                }
                else
                {
