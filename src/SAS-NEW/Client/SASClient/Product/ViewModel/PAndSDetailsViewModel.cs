...
namespace SDN.Product.ViewModel
{
    ...
    public class PAndSDetailsViewModel : ViewModelBase
    {
        ...
        #region "Constructors"
        public PAndSDetailsViewModel(IRegionManager regionManager, IEventAggregator eventAggregator)
        {
        ...
        private void RefreshPandSData()
        {
            ...
            {
                int i = 0;
                var LinkId = int.TryParse(SharedValues.getValue, out i);
                var refresh = ProductAndServices.Where(e => e.PSCode == SharedValues.getValue||e.PSName==SharedValues.getValue||e.ID== i).SingleOrDefault();
                if (refresh != null)
                {
        ...
        private void GetPandSData()
        {
            ...
            if (ProductAndServices != null)
            {
                ...
                if (current != null)
                {
                    ...
                    if (AverageCostPriceAfterGST != null)
                    {
                        StockValue = QuantityInStock * Convert.ToDecimal(AverageCostPriceAfterGST);
                    }
                    else
                    {
                        StockValue = QuantityInStock * Convert.ToDecimal(current.StandardCostpriceAfterGST);
                    }
                    TotalStockValue = Convert.ToDouble(StockValue);
                }
            }
            isNew = false;
