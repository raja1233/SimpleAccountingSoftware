    ...
    public class CompanyDAL : ICompanyDAL
    {
            ...
            using (var ts = entites.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (result != null)
                    {
                        ...
                        result.GST_Registered = companyDetail.AllowGstRegistered == null ? false : companyDetail.AllowGstRegistered;
                        entites.SaveChanges();
                        ...
                        ts.Commit();
                        GstRegistered = (from tab in entites.CompanyDetails where tab.IsDeleted == null select tab.GST_Registered).SingleOrDefault();
                        if(GstRegistered==true)
                        {
                            var status = entites.Accounts.SqlQuery("USP_UpdateGstPaid  @GstRegistered",
                                new SqlParameter("GstRegistered", GstRegistered));
                        }
                        else
                        {
                            var status = entites.Accounts.SqlQuery("USP_UpdateGstPaid  @GstRegistered",
                    ...
                    else
                    {
                        CompanyDetail CompDetail = new CompanyDetail()
                        {
                            ...
                            GST_Registered=companyDetail.AllowGstRegistered
        ...
        public CompanyDetailsEntities GetCompanyDetails()
        {
            ...
            if (source != null)
            {
                ...
                result.AllowGstRegistered = source.GST_Registered;
                if(source.GST_Registered == true)
                {
                    result.RegisterGstTrue = true;
                    result.RegisterGstFalse = false;
                }
                else
                {
                    result.RegisterGstTrue = false;
                    result.RegisterGstFalse = true;
                }
