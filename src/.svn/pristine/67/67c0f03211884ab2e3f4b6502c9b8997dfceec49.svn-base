...
namespace SDN.Sales.ViewModel
{
    ...
    public class SalesOrderListViewModel : SalesOrderListEntity
    {
        ...
        private void RefreshData()
        {
            ...
            this.JsonData = purchaseRepository.GetLastSelectionData(Convert.ToInt32(ScreenId.SalesOrderList));
            this.SalesOrderList = purchaseRepository.GetAllSalesOrderJson(this.JsonData, this.IncludingGSTTrue).OrderBy(x=>x.CustomerName).ToList();
            this.ShowSelectedCount = this.SalesOrderList.Count();
            ...
            if (SearchFilter != null || SearchFilter == string.Empty)
            {
                if (parameter == "Customer")
                {
                    ...
                    if (this.ShowAllTrue == true)
                    {
                        ...
                        this.SalesOrderList = this.SalesOrderList = FullPQList.Where(x => (x.CustomerName == SearchFilter || x.OrderNo == SearchFilter || x.ConvertedTo == SearchFilter)).OrderBy(x=>x.CustomerName).ToList();
                        //if (this.IncludingGSTTrue == true)
                        //{
                ...
                if (parameter == "ConvertedTo")
                {
                    ...
                    if (this.ShowAllTrue == true)
                    {
                        ...
                        if (search == " SO")
                            this.SalesOrderList = FullPQList.Where(x => x.ConvertedTo == "Order").OrderBy(x=>x.CustomerName).ToList();
                        else if (search == " SI")
                            this.SalesOrderList = FullPQList.Where(x => x.ConvertedTo == "Invoice").OrderBy(x => x.CustomerName).ToList();
                    }
                    else
                    {
                        var search = SearchFilter.Split(':')[1];
                        if (search == " SO")
                            this.SalesOrderList = DefaultList.Where(x => x.ConvertedTo == "Order").OrderBy(x => x.CustomerName).ToList();
                        else if (search == " SI")
                            this.SalesOrderList = DefaultList.Where(x => x.ConvertedTo == "Invoice").OrderBy(x => x.CustomerName).ToList();
                    }
